...
class OpkgRootfs(Rootfs):
    ...
    def _create(self):
        ...
        if self.split_debug_fs == "True":
            ...
            self.pm.opkg_args += self.pm.opkg_extra_debug_args
            bb.note("opkg_args before calling install_complementary: %s" % self.pm.opkg_args)
            self.pm.install_complementary(debug_fs_globs)
            ...
            self.pm.install(re.split(" +", extra_debug_packages), True)

            # revert the opkg_args
            self.pm.opkg_args = self.pm.opkg_args.replace(self.pm.opkg_extra_debug_args, "")
